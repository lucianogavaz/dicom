...
set SERVICE_NAME=DICOMGatewayWorker
set NSSM_EXE=C:\tools\nssm\win64\nssm.exe
...
%NSSM_EXE% install %SERVICE_NAME% "%PYTHON_EXE%" -m app.run_worker
%NSSM_EXE% set %SERVICE_NAME% AppDirectory "%APP_DIR%"
%NSSM_EXE% set %SERVICE_NAME% AppStdout "%LOG_DIR%\worker-service.out.log"
%NSSM_EXE% set %SERVICE_NAME% AppStderr "%LOG_DIR%\worker-service.err.log"
